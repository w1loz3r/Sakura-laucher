...
fn client() -> Result<reqwest::blocking::Client, String> {
    reqwest::blocking::Client::builder()
        .user_agent("SakuraLauncher/0.10.1")
...
#[tauri::command]
fn launcher_info()->Value{json!({"name":"Sakura Launcher","version":"0.10.0","status":"minecraft-core"})}
...
fn install_minecraft(app: tauri::AppHandle,id:String,version:String,loader:String)->Result<Value,String>{
    ...
    if let Some(ai)=meta.get("assetIndex"){
        ...
        if let Some(objects)=asset_index_data.get("objects").and_then(Value::as_object){
            ...
            for (_,o) in objects {if let Some(h)=o.get("hash").and_then(Value::as_str){if h.len()<2{continue;}let p=game.join("assets").join("objects").join(&h[0..2]).join(h);let u=format!("https://resources.download.minecraft.net/{}/{}",&h[0..2],h);jobs.push(DownloadJob{url:u,path:p,phase:"Ресурсы".into(),current:"Asset object".into(),sha1:Some(h.to_string())});}}
...
fn main(){
    tauri::Builder::default()
        .invoke_handler(tauri::generate_handler![launcher_info,create_instance,list_instance_files,open_instance,install_content,install_mod,install_modpack,install_minecraft,find_java,install_java,duplicate_instance,rename_instance,export_instance,repair_instance,launch_instance,rpc_update])
