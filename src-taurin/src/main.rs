...
fn client() -> Result<reqwest::blocking::Client, String> {
    reqwest::blocking::Client::builder()
        .user_agent("SakuraLauncher/0.9.0")
...
fn download_jobs_parallel(
    ...
    let count = workers.clamp(4, 8).min(
        queue
            .lock()
            .map_err(|e| e.to_string())?
            .len()
    ...
    for _ in 0..count {
        ...
        handles.push(thread::spawn(move || loop {
            ...
            for attempt in 1..=3 {
                match http_get_with_client(&http, &job.url) {
                    Ok(bytes) => {
                        let tmp = job
                            .path
...
fn safe_extract(
    ...
    {
        return Err(
            "Опасный путь внутри natives-архива".into()
        );
    }
...
fn install_content(
    ...
    if !allowed_ext.iter().any(|ext| {
        safe.to_lowercase().ends_with(ext)
    }) {
        return Err(format!(
...
fn install_modpack(
    ...
    let index_file =
        zip.by_name("modrinth.index.json")
            .map_err(|_| {
                "В .mrpack нет modrinth.index.json"
                    .to_string()
            })?;
    ...
    let mc = deps
        ...
        .and_then(Value::as_str)
        .ok_or(
            "Modpack не указал версию Minecraft",
        )?
        .to_string();
...
fn install_minecraft(
    ...
    if loader.eq_ignore_ascii_case("fabric") {
        ...
        let arr =
            http_json(&versions_url)?
                .as_array()
                .cloned()
                .ok_or(
                    "Fabric Meta вернул неверный ответ"
                )?;
        ...
        let chosen = arr
            ...
            .ok_or(
                "Для этой версии нет Fabric Loader"
            )?;
        ...
        let lv = chosen
            ...
            .and_then(Value::as_str)
            .ok_or(
                "Не найден Fabric Loader version"
            )?;
    ...
    let mut asset_index_data =
        Value::Null;

    let mut asset_index_url =
        None;

    let mut asset_objects_count =
        0usize;
...
fn install_java(
    ...
    for i in 0..z.len() {
        ...
        {
            return Err(
                "Некорректный путь в Java archive"
                    .into()
            );
        }
    ...
    let found =
        find_java_under(&dir)
            .ok_or(
                "Java скачана, но executable не найден"
            )?;
...
fn launch_instance(
    ...
    let java =
        ...
        .ok_or(
            "Java не найдена. Укажи путь к Java в настройках или установи Java 21."
        )?;
    ...
    if loader.eq_ignore_ascii_case(
        "fabric",
    ) {
        let fp =
            vdir.join("fabric.json");
    ...
    let mut child =
        Command::new(java)
            ...
            .map_err(|e| {
                format!(
                    "Не удалось запустить Minecraft через Java: {e}"
                )
            })?;
...
fn main() {
    tauri::Builder::default()
        ...
        .expect(
            "error while running Sakura Launcher"
        );
}
