...
fn download_jobs_parallel(
    ...
    let count = workers
        .clamp(2, 16)
        .min(queue.lock().map_err(|e| e.to_string())?.len().max(1));
    ...
    for _ in 0..count {
        ...
        handles.push(thread::spawn(move || loop {
            ...
            emit_progress(&app, &job.phase, &job.current, n, total);
            ...
            for attempt in 1..=3 {
                match http_get(&job.url) {
                    Ok(bytes) => {
                        let tmp = job
                            .path
                            .with_extension(format!("part{}", attempt));

                        match fs::write(&tmp, bytes)
                            .map_err(|e| e.to_string())
                            .and_then(|_| {
                                fs::rename(&tmp, &job.path)
                                    .map_err(|e| e.to_string())
                            })
                        {
                            Ok(_) => {
    ...
    for h in handles {
        h.join()
            .map_err(|_| "Поток загрузки завершился с ошибкой".to_string())?;
    }
...
fn download_library_list(
    ...
    *completed =
        download_jobs_parallel(app, jobs, *completed, total, 12)?;
    ...
    #[cfg(windows)]
    {
        ...
        for root in roots {
            ...
            for entry in entries.flatten() {
                ...
                if !dir.is_dir() || !checked.insert(dir.clone()) {
                    continue;
                }
...
fn install_content(
    ...
    let (folder, label, allowed_ext): (&str, &str, &[&str]) =
    ...
    if !allowed_ext
        .iter()
        .any(|ext| safe.to_lowercase().ends_with(ext))
    {
        return Err(format!(
            "Файл не подходит для типа {}",
            content_type
        ));
    }

    let dir = instances_root(&app)?.join(&id).join(folder);
...
fn safe_join(root: &Path, rel: &str) -> Result<PathBuf, String> {
    ...
    if p.is_absolute() || rel.split('/').any(|x| x == "..") {
        return Err(format!("Опасный путь в modpack: {}", rel));
    }
...
fn install_modpack(
    ...
    let mut zip = zip::ZipArchive::new(Cursor::new(bytes))
        .map_err(|e| format!("Не удалось открыть modpack: {}", e))?;

    let index_file = zip
        .by_name("modrinth.index.json")
        .map_err(|_| {
            "В .mrpack нет modrinth.index.json".to_string()
        })?;

    let index: Value = serde_json::from_reader(index_file)
        .map_err(|e| {
            format!("Неверный modrinth.index.json: {}", e)
        })?;

    let deps = index
        .get("dependencies")
        .and_then(Value::as_object)
        .ok_or("В modpack нет dependencies")?;

    let mc = deps
        .get("minecraft")
        .and_then(Value::as_str)
        .ok_or("Modpack не указал версию Minecraft")?
        .to_string();
    ...
    emit_progress(
        &app,
        "Modpack",
        "Устанавливаю Minecraft для сборки…",
        0,
        0,
    );
    ...
    for f in &files {
        ...
        if env.eq_ignore_ascii_case("unsupported") {
            continue;
        }
        ...
        let path = f
            .get("path")
            .and_then(Value::as_str)
            .ok_or("У modpack-файла нет path")?;

        let downloads = f
            .get("downloads")
            .and_then(Value::as_array)
            .ok_or(format!(
                "У modpack-файла нет downloads: {}",
                path
            ))?;
...
fn install_minecraft(
    ...
    let vdir = game.join("versions").join(&version);
    ...
    let asset_count =
        asset_index_url.as_ref().map(|_| 1usize).unwrap_or(0);
    ...
    if let Some(ai) = meta.get("assetIndex") {
        ...
        let idx = game.join("assets").join("indexes");
            ...
            for (_, o) in objects {
                    ...
                    o.get("hash").and_then(Value::as_str)
                {
                    ...
                    jobs.push(DownloadJob {
                        ...
                        path: p,
                        phase: "Ресурсы".into(),
                        current: "Asset object".into(),
                    });
                }
            }

            completed = download_jobs_parallel(
                &app,
                jobs,
                completed,
                total,
    ...
    Ok(json!({
        ...
        "version": version,
        "loader": loader,
        "loaderInfo": if loader.eq_ignore_ascii_case("fabric") {
            json!({
                "name": "Fabric",
...
fn install_java(
    ...
    let archive = dir.join("java21.zip");
    ...
    for i in 0..z.len() {
        ...
        if name.starts_with('/') || name.contains("../") {
            return Err(
                "Некорректный путь в Java archive".into()
            );
        }
    ...
    let found = find_java_under(&dir)
        .ok_or("Java скачана, но executable не найден")?;
...
fn find_java_under(root: &Path) -> Option<String> {
        ...
        if let Ok(rd) = fs::read_dir(d) {
            for e in rd.flatten() {
                ...
                if p.is_dir() {
                    ...
                    .and_then(|x| x.to_str())
                    .map(|x| {
                        x.eq_ignore_ascii_case("javaw.exe")
                            || x.eq_ignore_ascii_case("java.exe")
                    })
                    .unwrap_or(false)
                    && java_is_usable(&p)
                {
...
fn duplicate_instance(
    ...
    let src = instances_root(&app)?.join(&id);

    let dst = instances_root(&app)?.join(slug(&new_id));
    ...
    if dst.exists() {
        return Err("Такая сборка уже существует".into());
    }
...
fn rename_instance(
    ...
    let clean = slug(&new_id);
    let dst = instances_root(&app)?.join(&clean);
...
fn launch_instance(
    ...
    if let Some(extra) = opts.jvm_args {
        jvm_args.extend(
            extra
                .split_whitespace()
                .map(str::to_string),
        ...
        (opts.width, opts.height)
    {
        game_args.extend([
            "--width".into(),
            w.clamp(640, 7680).to_string(),
            "--height".into(),
            h.clamp(480, 4320).to_string(),
        ]);
    }
