...
use std::{collections::HashMap, fs, io::{self, Cursor, Read}, path::{Path, PathBuf}, process::{Command, Stdio}};
...
fn download_library_list(app: &tauri::AppHandle, libs: &[Value], libraries: &Path, phase: &str, completed: &mut usize, total: usize) -> Result<(), String> {
    for lib in libs {
        ...
        if let Some(p)=artifact_path(lib,libraries) {
            if let Some(u)=lib.get("downloads").and_then(|x|x.get("artifact")).and_then(|x|x.get("url")).and_then(Value::as_str) { download_to(app,u,&p,phase,&format!("{}",p.file_name().unwrap_or_default().to_string_lossy()),completed,total)?; }
        } else if let Some(name)=lib.get("name").and_then(Value::as_str) {
            if let Some(base)=lib.get("url").and_then(Value::as_str) {
                if let Some(path)=maven_path(name) { download_to(app,&format!("{}{}",base,path),&libraries.join(&path),phase,&path,completed,total)?; }
            }
        }
    }
    Ok(())
...
fn java_from_path(path: Option<&str>) -> Option<String> {
    if let Some(p)=path { if !p.trim().is_empty() && Path::new(p).exists() { return Some(p.to_string()); } }
    for name in ["javaw.exe","java.exe","java"] {
        if let Ok(o)=Command::new(name).arg("-version").output(){ if o.status.success(){ return Some(name.to_string()); } }
    }
    None
}

#[tauri::command]
fn launcher_info()->Value{json!({"name":"Sakura Launcher","version":"0.5.0","status":"minecraft-core"})}
...
fn install_mod(app:tauri::AppHandle,id:String,url:String,filename:String)->Result<(),String>{
    ...
    if !safe.to_lowercase().ends_with(".jar"){return Err("Modrinth-файл не является .jar".into());}
    let dir=instances_root(&app)?.join(&id).join("mods"); fs::create_dir_all(&dir).map_err(|e|e.to_string())?;
    let mut completed=0usize; download_to(&app,&url,&dir.join(safe),"Мод","Установка мода",&mut completed,1)
}
...
fn install_minecraft(app: tauri::AppHandle,id:String,version:String,loader:String)->Result<Value,String>{
    ...
    if let Some(ai)=meta.get("assetIndex"){
        ...
        if let Some(objects)=asset_index_data.get("objects").and_then(Value::as_object){
            for (_,o) in objects {if let Some(h)=o.get("hash").and_then(Value::as_str){if h.len()<2{continue;}let p=game.join("assets").join("objects").join(&h[0..2]).join(h);let u=format!("https://resources.download.minecraft.net/{}/{}",&h[0..2],h);download_to(&app,&u,&p,"Ресурсы","Asset object",&mut completed,total)?;}}
        }
    }
...
#[tauri::command]
fn launch_instance(app:tauri::AppHandle,id:String,version:String,username:String,loader:String,java_path:Option<String>)->Result<(),String>{
    let instance=instances_root(&app)?.join(&id); if !instance.exists(){return Err("Сборка не найдена".into());}
    ...
    let java=java_from_path(java_path.as_deref()).ok_or("Java не найдена. Укажи путь к javaw.exe в настройках или установи Java.")?;
    ...
    vars.insert("launcher_version".into(),"0.5.0".into());
    let mut cp=Vec::new();
    ...
    let mut args=Vec::new(); args.extend(jvm_args); args.push("-Djava.library.path=${natives_directory}".replace("${natives_directory}",&vars["natives_directory"])); args.push("-cp".into()); args.push(cp.join(";")); args.push(main_class); args.extend(game_args);
    Command::new(java).args(args).current_dir(&game).stdin(Stdio::null()).stdout(Stdio::null()).stderr(Stdio::null()).spawn().map_err(|e|format!("Не удалось запустить Java: {e}"))?;
    Ok(())
}

fn main(){
    tauri::Builder::default()
        .invoke_handler(tauri::generate_handler![launcher_info,create_instance,list_instance_files,open_instance,install_mod,install_minecraft,find_java,launch_instance])
