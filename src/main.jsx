...
function App(){
 ...
 async function addMod(project){
   ...
   try{
     ...
     const loader=normalizeLoader(target,selected.loader||modLoader).toLowerCase();
     ...
     if(!file)throw new Error(`Нет версии ${target} для ${selected.loader||'этого загрузчика'}`);
     setStatus(`Устанавливаю ${project.title}…`);
     setDownload({phase:isPack?'Modpack':'Мод',current:file.filename,completed:0,total:1,percent:0});
     showDownload=true;
     if(isPack){
       const result=await invoke('install_modpack',{id:selected.id,url:file.url,filename:file.filename});
       const b={...selected,version:result.version,loader:result.loader,installed:true};
       setSelected(b);setBuilds(xs=>xs.map(x=>x.id===selected.id?b:x));
     ...
     const next={...selected,mods:[...new Set([...(selected.mods||[]),file.filename])],modsMeta:[...(selected.modsMeta||[]),{projectId:project.project_id,filename:file.filename,versionId:v.id,loader,gameVersion:target}]};
     setSelected(next);setBuilds(xs=>xs.map(x=>x.id===selected.id?next:x));
     setStatus(`${project.title} установлен.`);
   }catch(e){
     setStatus(`Ошибка установки: ${String(e)}`);
   }finally{
 ...
 return <div className={`app theme-${settings.theme} ${settings.glow?'glow-on':''} ${settings.animations?'animations-on':''}`} style={appStyle}>
   ...
   <aside><div className="brand"><img src={logo} alt="Sakura"/><div><b>SAKURA</b><span>CRAFT LAUNCHER</span></div></div>
    ...
    <div className="sidebottom"><button onClick={()=>setModal('settings')}>⚙ Настройки</button><small>v0.10.0 • minecraft core</small></div>
   </aside>
   ...
   {modal==='settings'&&<SettingsModal profile={profile} setStatus={setStatus} settings={settings} setSettings={setSettings} close={()=>setModal(null)} reset={()=>setSettings(defaultSettings)} installJava={installJava} javaInstalling={javaInstalling}/>} 
